...
class DDPG(object):
    ...
    def evaluate(self, state, action, target=False):
        ...
        if target:
#             Q,Q1 = self.critic_target(merged_state)
#             Q = torch.min(Q,Q1)
            Q = self.critic_target(merged_state)
          
            return (Q + gan_reward), gan_reward
        else:
#             Q,Q1 = self.critic(merged_state)
            Q = self.critic_target(merged_state)
            if self.log % 20 == 0:
...
#         cur_q,cur_q1, step_reward = self.evaluate(state, action)
        cur_q,step_reward = self.evaluate(state, action)
        target_q += step_reward.detach()
        
#         value_loss = criterion(cur_q, target_q) + criterion(cur_q1, target_q)
